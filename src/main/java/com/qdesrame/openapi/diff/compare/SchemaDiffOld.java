...
public class SchemaDiffOld {
    ...
    private SchemaDiffOld() {
        increased = new ArrayList<ElSchema>();
        missing = new ArrayList<ElSchema>();
    }

    public static SchemaDiffOld buildWithDefinition(Map<String, Schema> left,
                                                    Map<String, Schema> right) {
        SchemaDiffOld diff = new SchemaDiffOld();
        diff.oldDefinitions = left;
        diff.newDefinitions = right;
        return diff;
    }

    public SchemaDiffOld diff(Schema leftSchema, Schema rightSchema) {
        return this.diff(leftSchema, rightSchema, null);
    }

    public SchemaDiffOld diff(Schema leftSchema, Schema rightSchema, String parentEl) {
